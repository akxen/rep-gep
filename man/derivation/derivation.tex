...
\newcommand{\vInstalledCapacityStartupStateAux}[1][\iGenerator,\iYear,\iScenario,\iInterval+1]{y_{#1}}
...
\subsubsection{Program for first year and first operating scenario}
Note that all complicating constraints will be included in this scenario. The objective function of this sub-problem has the form:
...
\begin{equation}
	\sScenarios^{\prime} = \begin{dcases}
	\sScenarios \setminus \{1\} & \mathrm{if} \quad \iYear = 1\\
...
Unit commitment constraints for the given operating scenario, (\ref{eqn: reserve constraints})--(\ref{eqn: powerflow constraints}). Investment constraints (\ref{eqn: candidate thermal unit capacity})--(\ref{eqn: storage build limits}):
...
\begin{equation}
\vInstalledCapacityTotalScenario[\iGenerator,1,1] - \vInstalledCapacityTotal[\iGenerator,1] = 0 \quad (\psi_{\iGenerator,1,1}) \quad \forall \iGenerator \in \sGeneratorsCandidate
\label{eqn: first interval variable capacity}
\end{equation}


policy constraint (\ref{eqn: cumulative emissions target}).

\begin{equation}
\sum\limits_{\iYear \in \sYears} \sum\limits_{\iScenario \in \sScenarios^{\prime}} \cScenarioDuration \sum\limits_{\iInterval \in \sIntervals} \sum\limits_{\iGenerator \in \sGeneratorsThermal} \overline{\vEnergy[]}_{\iGenerator,\iYear,\iScenario,\iInterval} \cEmissionsIntensity + \cScenarioDuration[1,1]\sum\limits_{\iInterval \in \sIntervals}\sum\limits_{\iGenerator \in \sGeneratorsThermal} \vEnergy[\iGenerator,1,1,\iInterval] \cEmissionsIntensity - \cEmmissionsCumulativeTarget - \vEmissionsTargetViolation \leq 0 \quad (\lambda)
...
\subsubsection{Program for all other operating scenarios}
Note: no complicating constraints are explicitly included for these sub-problems, but that they are implicitly included within the objective function:

\begin{align}
	\begin{split}
		\minimise\limits_{\bm{x}} \quad &\cObjectiveFunction + \sum\limits_{\iGenerator \in \sGeneratorsCandidate}\sum\limits_{\iYear \in \sYears}\sum\limits_{\iScenarioAlias=1,m\neq \iScenario}^{\sScenarios} \overline{\psi}_{\iGenerator,\iYear,\iScenario}\left[\overline{\vInstalledCapacityTotalScenario[]}_{\iGenerator,\iYear,\iScenario} - \overline{\vInstalledCapacityTotal[]}_{\iGenerator,\iYear}\right]\\
		& + \overline{\lambda} \left[\sum\limits_{\iYear \in \sYears} \sum\limits_{\iScenarioAlias=1, \iScenarioAlias\neq\iScenario}^{\sScenarios} \cScenarioDuration \sum\limits_{\iInterval \in \sIntervals} \sum\limits_{\iGenerator \in \sGeneratorsThermal} \overline{\vEnergy[]}_{\iGenerator,\iYear,\iScenario,\iInterval} \cEmissionsIntensity + \cScenarioDuration\sum\limits_{\iInterval \in \sIntervals}\sum\limits_{\iGenerator \in \sGeneratorsThermal} \vEnergy[\iGenerator,\iYear,\iScenario,\iInterval] \cEmissionsIntensity - \cEmmissionsCumulativeTarget - \overline{\vEmissionsTargetViolation}\right]\\
	\end{split}
\end{align}
...
\begin{equation}
	k_{a} = \sum\limits_{\iGenerator \in \sGeneratorsCandidate \cup \sStorageCandidate}\sum\limits_{\iYear \in \sYears}\sum\limits_{\iScenarioAlias=1,m\neq \iScenario}^{\sScenarios} \overline{\psi}_{\iGenerator,\iYear,\iScenario}\left[\overline{\vInstalledCapacityTotalScenario[]}_{\iGenerator,\iYear,\iScenario} - \overline{\vInstalledCapacityTotal[]}_{\iGenerator,\iYear}\right]
\end{equation}

\begin{equation}
	k_{b} = \sum\limits_{\iYear \in \sYears} \sum\limits_{\iScenarioAlias=1, \iScenarioAlias\neq\iScenario}^{\sScenarios} \cScenarioDuration \sum\limits_{\iInterval \in \sIntervals} \sum\limits_{\iGenerator \in \sGeneratorsThermal} \overline{\vEnergy[]}_{\iGenerator,\iYear,\iScenario,\iInterval} \cEmissionsIntensity - \cEmmissionsCumulativeTarget - \overline{\vEmissionsTargetViolation}
\end{equation}
...
\begin{align}
\begin{split}
k_{c} = & \sum\limits_{\iYear \in \sYears} \frac{(1+\cInterestRate)^{-\iYear}}{\cInterestRate}\overline{\cInvestmentCost[]}_{\iYear} + \sum\limits_{\iYearAlias=1, \iYearAlias \neq \iYear}^{\sYears} \left[(1+\cInterestRate)^{-\iYearAlias} (\overline{\cOperatingCost[]}_{\iYearAlias} + \overline{\cFixedOperationsMaintenanceCost[]}_{\iYearAlias}) \right]\\
...
As the constant term will not influence the optimal point it can be dropped from the objective. Therefore the objective function used to solve a given operating scenario (excluding the first and last years) is:
...
Constraints (\ref{eqn: on state aux variable block start})--(\ref{eqn: on state aux variable block end}) and (\ref{eqn: startup state aux variable block start})--(\ref{eqn: startup state aux variable block end}) are also included in each sub-problem to constrain the values auxiliary variables can take on.
