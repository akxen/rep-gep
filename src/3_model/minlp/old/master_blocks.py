...
class MasterProblem(BaseComponents):
    ...
    def define_constraints(self, m):
        ...
        # TODO: have moved these constraints defining binary variable logic from sub-problem
        ...
        def minimum_off_time_rule(_s, g, t):
            ...
            if g in self.existing_units.index:
                ...
                hours = self.existing_units_dict[('PARAMETERS', 'MIN_OFF_TIME')][g]

            # Hours for candidate units
            elif g in self.candidate_units.index:
                # hours = self.candidate_units.loc[g, ('PARAMETERS', 'MIN_OFF_TIME')]
                hours = self.candidate_units_dict[('PARAMETERS', 'MIN_OFF_TIME')][g]
